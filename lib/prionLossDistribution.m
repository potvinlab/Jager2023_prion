function [lostTime,keptTime,lostCell,lostIndex,keptCell,keptIndex,rejectedCell, rejectedIndex,neverCell,neverIndex, frame_index, loss_curve] = prionLossDistribution(dataCell,plots,oldPole,startTime)
...
for i=1:length(dataCell.spotNo)
    ...
    if dataCell.frameNo{i}(1)<20   && median(dataCell.YFPAvg{i}(ind)) > 10
        ...
        if sum(dataCell.spotNo{i}(5:20)>=1)>=3 
            ...
            is_old_pole = ~isempty(find(min_move<45,1,'first'));
